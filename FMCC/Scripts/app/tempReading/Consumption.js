angular.module("fmcc")
    .controller("Consumption", function ($scope, $rootScope, $routeParams, TempReading, ready) {
        var local = {};
        local.period = "sofartoday";
        local.type = $routeParams.type;
        local.field = $routeParams.field;
        ...
        local.view = {
            SoFarToday: function (res) {
                ...
                overall.css({ width: "665px", height: "400px", opacity: 1 });
                overall.highcharts(local.chartoption(function (config) {
                    ...
                    config.tooltip.headerFormat = "<span>" + moment().format("DD/MM/YYYY") + " {point.key}:00</span><table>";
                ...
                json.Overall.Reading.forEach(function (item) {
                    ...
                    switch (key) {
                        case 1:
                            ...
                            overallTempData[6] = [moment().isoWeekday(7).format("DD/MM/YYYY") + " " + "(Sunday)", sum];
                            break;
                        case 2:
                            overallTempCategory[0] = "Mon";
                            overallTempData[0] = [moment().isoWeekday(1).format("DD/MM/YYYY") + " " + "(Monday)", sum];
                            break;
                        case 3:
                            overallTempCategory[1] = "Tue";
                            overallTempData[1] = [moment().isoWeekday(2).format("DD/MM/YYYY") + " " + "(Tuesday)", sum];
                            break;
                        case 4:
                            overallTempCategory[2] = "Wed";
                            overallTempData[2] = [moment().isoWeekday(3).format("DD/MM/YYYY") + " " + "(Wednesday)", sum];
                            break;
                        case 5:
                            overallTempCategory[3] = "Thu";
                            overallTempData[3] = [moment().isoWeekday(4).format("DD/MM/YYYY") + " " + "(Thursday)", sum];
                            break;
                        case 6:
                            overallTempCategory[4] = "Fri";
                            overallTempData[4] = [moment().isoWeekday(5).format("DD/MM/YYYY") + " " + "(Friday)", sum];
                            break;
                        case 7:
                            overallTempCategory[5] = "Sat";
                            overallTempData[5] = [moment().isoWeekday(6).format("DD/MM/YYYY") + " " + "(Satarday)", sum];
                            break;
                        default:
                            break;
                    }
                    ...
                    individual.Reading.forEach(function (item) {
                        ...
                        switch (key) {
                            case 1:
                                ...
                                individualTempData[6] = [moment().isoWeekday(7).format("DD/MM/YYYY") + " " + "(Sunday)", sum];
                                break;
                            case 2:
                                individualTempCategory[0] = "Mon";
                                individualTempData[0] = [moment().isoWeekday(1).format("DD/MM/YYYY") + " " + "(Monday)", sum];
                                break;
                            case 3:
                                individualTempCategory[1] = "Tue";
                                individualTempData[1] = [moment().isoWeekday(2).format("DD/MM/YYYY") + " " + "(Tuesday)", sum];
                                break;
                            case 4:
                                individualTempCategory[2] = "Wed";
                                individualTempData[2] = [moment().isoWeekday(3).format("DD/MM/YYYY") + " " + "(Wednesday)", sum];
                                break;
                            case 5:
                                individualTempCategory[3] = "Thu";
                                individualTempData[3] = [moment().isoWeekday(4).format("DD/MM/YYYY") + " " + "(Thursday)", sum];
                                break;
                            case 6:
                                individualTempCategory[4] = "Fri";
                                individualTempData[4] = [moment().isoWeekday(5).format("DD/MM/YYYY") + " " + "(Friday)", sum];
                                break;
                            case 7:
                                individualTempCategory[5] = "Sat";
                                individualTempData[5] = [moment().isoWeekday(6).format("DD/MM/YYYY") + " " + "(Satarday)", sum];
                                break;
                            default:
                                break;
                        }
            ...
            SoFarThisMonth: function (res) {
                ...
                json.Overall.Reading.forEach(function (item) {
                    ...
                    //category and data
                    overallCategory.push(key);
                    overallData.push([moment().date(parseInt(key)).format("DD/MM/YYYY"), sum]);
                    overallAverage.push({ y: json.Overall.Average });
                    //overall hourly
                    omin = sum < omin ? sum : omin;
                    omax = sum > omax ? sum : omax;
                ...
                json.Individual.forEach(function (individual) {
                    ...
                    individual.Reading.forEach(function (item) {
                        ...
                        //category and data
                        individualCategory.push(key);
                        individualData.push([moment().date(parseInt(key)).format("DD/MM/YYYY"), sum]);
                        individualAverage.push({ y: individual.Average });
                        //overall hourly
                        imin = sum < imin ? sum : imin;
                        imax = sum > imax ? sum : imax;
        ...
        local.template = function (options) {
            var tpl = $('<div class="col-md-6" data-ng-repeat="block in consumption.Individual" data-ng-show="block.IsActive">' +
                      '<div class="box box-warning direct-chat direct-chat-warning">' +
                        ...
                        '</div>' +
                        '<div class="box-body">' +
                          '<div class="small-chart small-chart-hourly" style="width: 460px; height: 300px; margin-top: 20px;"></div>' +
                          '<div class="small-chart small-chart-daily" style="width: 460px; height: 300px; margin-top: 20px;"></div>' +
                          '<div class="small-chart small-chart-weekly" style="width: 460px; height: 300px; margin-top: 20px;"></div>' +
                          '<div class="small-chart small-chart-monthly" style="width: 460px; height: 300px; margin-top: 20px;"></div>' +
                          '<div class="small-chart small-chart-yearly" style="width: 460px; height: 300px; margin-top: 20px;"></div>' +
                        '</div>' +
                      '</div>' +
                   '</div>');
            return tpl;
        ...
        $scope.type.select = function (_event, _type) {
            ...
            if (!elm.hasClass("active")) {
                switch (_type) {
                    case "sofartoday":
                        local.container.overall.html("<div class='mask' style='display:block;'></div>")
                        local.period = "sofartoday";
                        $scope.chart.unit = $scope.view.chartUnit + "/h";
                        local.active.removeClass("active");
                        local.active = elm.addClass("active");
                        local.container.individual.html("");
                        TempReading.Get(local.type, local.field, local.period, local.view.SoFarToday, local.view.failure);
                        break;
                    case "sofarthismonth":
                        local.container.overall.html("<div class='mask' style='display:block;'></div>")
                        ...
                        local.active = elm.addClass("active");
                        local.container.individual.html("");
                        TempReading.Get(local.type, local.field, local.period, local.view.SoFarThisMonth, local.view.failure);
                        break;
                    case "sofarthisweek":
                        local.container.overall.html("<div class='mask' style='display:block;'></div>")
                        ...
                        local.active = elm.addClass("active");
                        local.container.individual.html("");
                        TempReading.Get(local.type, local.field, local.period, local.view.SoFarThisWeek, local.view.failure);
                        break;
                    case "sofarthisyear":
                        local.container.overall.html("<div class='mask' style='display:block;'></div>")
                        ...
                        local.active = elm.addClass("active");
                        local.container.individual.html("");
                        TempReading.Get(local.type, local.field, local.period, local.view.SoFarThisYear, local.view.failure);
                        break;
                    case "yearly":
                        local.container.overall.html("<div class='mask' style='display:block;'></div>")
                        local.period = "yearly";
                        ...
                        local.active = elm.addClass("active");
                        local.container.individual.html("");
                        TempReading.Get(local.type, local.field, local.period, local.view.yearly, local.view.failure);
                        break;
                    default:
                        break;
                }
